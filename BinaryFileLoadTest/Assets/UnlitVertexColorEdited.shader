						...
						else//FACES PASSED IN, NEED PROCESSING
						{
							...
							for (int a = 0; a < 500; a++)
							{
								...
								//{//IF CURRENT FACE DRAWING POINT IS NOT NULL
									...
									if (input.pos.x * 0.025 >= x)
								    //if (o.position.x >= x)

									{
										xDifference = input.pos.x * 0.025 - x;
										//xDifference = o.position.x - x;

									}
									else
									{
										xDifference = x - input.pos.x * 0.025;
										//xDifference = x - o.position.x;

									}

									if (input.pos.y * 0.025 >= y)
								    //if (o.position.y >= y)

									{
										yDifference = input.pos.y * 0.025 - y;
										//yDifference = o.position.y - y;

									}
									else
									{
										yDifference = y - input.pos.y * 0.025;
										//yDifference = y - o.position.y;

									}

									if (input.pos.z * 0.025 >= z)
									//if (o.position.z >= z)

									{
										zDifference = input.pos.z * 0.025 - z;
										//zDifference = o.position.z - z;

									}
									else
									{
										zDifference = z - input.pos.z * 0.025;
										//zDifference = z - o.position.z;

									}

									if (xDifference < 0.01 &&//1 works on PC
										yDifference < 0.01 &&//1 works on PC
										zDifference < 0.01//1 works on PC
										)
									{
										o.color = float4(0, 255, 0, 255);
										//c = 255;
